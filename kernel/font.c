 ...
 *
 * 8x16 VGA/BIOS bitmap font for framebuffer rendering.
 * Each character is 8 pixels wide (1 byte per row) and 16 rows tall.
 */
...
/*============================================================================
 * 8x16 VGA Font Data (ASCII 32-126)
 * Standard PC BIOS font, 16 bytes per character.
 * Each byte represents one row: MSB = leftmost pixel.
 *============================================================================*/

const uint8_t font_data[95][16] = {
    /* 32: ' ' (space) */
...
void font_draw_char(uint32_t x, uint32_t y, char ch, uint32_t fg, uint32_t bg)
{
    ...
    if (!backbuf || x + FONT_WIDTH > fb_w || y + FONT_HEIGHT > fb_h)
        return;
